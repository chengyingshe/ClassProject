...
class PTV3Encoder(PointModule):
    ...
    def forward(self, data):
        if not isinstance(data, torch.Tensor):
            ...
            data_tensor = data
        feat = data_tensor
        coord = data_tensor[:, :3].contiguous()
        offset = torch.tensor([data_tensor.size(0)]).to(data_tensor.device)
        batch = offset2batch(offset)
        return self._forward_with_data_dict(
                Point(coord=coord, 
                        feat=feat, 
                        offset=offset,
                        batch=batch,
    ...
    def _forward_with_data_dict(self, data_dict):
        ...
        #         print(f'####{k}:', point[k].shape)
        #     else:
        #         print(f'####{k}:', point[k].shape)
        # print('####feat:', point.feat)
        # print('####sparse_conv_feat:', point.sparse_conv_feat.features)
        # feat = torch.concat([point.feat, point.sparse_conv_feat], dim=1)
        feat = point.feat
        return feat
